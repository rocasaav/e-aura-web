...
function PersonalizerContent() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        setAromasList(aromas);
        const defaultAroma = aromas.length > 0 ? aromas[0].name : 'Neutro';
        ...
        const savedCustoms: SavedCustomization[] = savedCustomsRaw ? JSON.parse(savedCustomsRaw) : [];

        const initialCustomizations: SavedCustomization[] = favIds.map((favId) => {
          const existing = savedCustoms.find((c) => c.productId === favId);
          if (existing) return existing;

          const prod = formattedCatalog.find((p) => p.id === favId);
          return {
            id: favId.toString(),
            productId: favId,
            productName: prod ? prod.name : `Producto ${favId}`,
            color: 'Blanco Marfil',
            colorHex: '#fdfbf7',
            aroma: defaultAroma,
            quantity: 1,
            unitPrice: prod ? prod.price : 0,
            total: prod ? prod.price * 1 : 0,
          };
        });
  ...
  const handleSaveConfiguration = () => {
    ...
    setCustomizations((prev) => {
      ...
      localStorage.setItem('eaura_customizations', JSON.stringify(updated));
      return updated;
    });
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-12">
      {/* CABECERA Y SELECTOR DE FAVORITOS */}
      ...
      {/* PESTAÑAS PARA CAMBIAR ENTRE FAVORITOS */}
      {favoriteProducts.length > 1 && (
        <div className="flex items-center gap-2 overflow-x-auto pb-2">
          <span className="text-xs text-[#a3685e] font-bold font-[var(--font-cinzel)] whitespace-nowrap">
            Editar otro favorito:
          </span>
          {favoriteProducts.map((fav) => (
            <button
              key={fav.id}
              onClick={() => setProduct(fav)}
              className={`text-xs px-4 py-1.5 rounded-full font-[var(--font-cinzel)] whitespace-nowrap transition-all ${
                product.id === fav.id
                  ? 'bg-gradient-to-r from-[#a3685e] to-[#8c5349] text-white font-bold shadow-xs'
                  : 'bg-white/80 text-[#5c423d] hover:bg-[#fcf5f3] border border-[#eee2de]'
              }`}
            >
              {fav.name}
            </button>
      ...
      <div className="bg-white/70 backdrop-blur-md rounded-2xl p-4 border border-[#f0e4e1] shadow-xs flex flex-col md:flex-row items-center gap-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
        ...
        <div className="bg-white/70 backdrop-blur-md rounded-2xl p-6 border border-[#f0e4e1] shadow-xs space-y-6">
          ...
          <div className="space-y-3 pt-2">
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="flex-1 bg-gradient-to-r from-[#a3685e] to-[#8c5349] hover:from-[#965a50] hover:to-[#7d473d] text-white text-xs font-bold py-3 px-4 rounded-xl font-[var(--font-cinzel)] tracking-wider transition-all shadow-xs hover:shadow-md active:scale-[0.99]"
              >
          ...
          {savedMessage && (
            <p className="text-center text-xs text-rose-900 bg-rose-50 border border-rose-200 p-2.5 rounded-xl font-medium animate-in fade-in">
              ✓ Configuración actualizada para {product.name}.
            </p>
          )}
        </div>
      </div>
      ...
      {customizations.length > 0 && (
        <div className="bg-white/70 backdrop-blur-md rounded-2xl p-6 border border-[#f0e4e1] shadow-xs space-y-5">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            ...
            <Link
              href="/"
              className="text-xs bg-white text-[#8c5349] hover:bg-[#fcf5f3] border border-[#e8d5d1] px-4 py-2 rounded-xl font-[var(--font-cinzel)] transition-all shadow-2xs font-bold tracking-wider"
            >
              ← Seguir eligiendo en el catálogo
            </Link>
          </div>
          ...
          <div className="bg-[#FAF5F7]/90 backdrop-blur-md rounded-3xl p-6 md:p-8 border border-[#E8DCE2] shadow-xs space-y-6 font-[var(--font-montserrat)] text-[#4a3531]">
            <div className="flex items-center justify-between border-b border-[#E8DCE2]/80 pb-4">
              ...
              <span className="text-[10px] md:text-xs font-semibold px-3 py-1 rounded-full bg-gradient-to-r from-[#a3685e] to-[#8c5349] text-white shadow-2xs font-[var(--font-cinzel)] tracking-wider">
                Hecho a tu medida
              </span>
            </div>
              ...
              <table className="w-full text-center text-xs border-collapse table-fixed">
                <thead>
                  <tr className="border-b border-[#E8DCE2] text-[#8c5349] uppercase font-[var(--font-cinzel)] tracking-wider">
                    <th className="w-[18%] py-2.5 px-3 font-bold">Artículo</th>
                    <th className="w-[15%] py-2.5 px-3 font-bold">Color</th>
                    <th className="w-[20%] py-2.5 px-3 font-bold">Aroma</th>
                    <th className="w-[15%] py-2.5 px-3 font-bold">Precio Unidad</th>
                    <th className="w-[12%] py-2.5 px-3 font-bold text-center">Cantidad</th>
                    <th className="w-[20%] py-2.5 px-3 font-bold text-center">Subtotal</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#E8DCE2]/50">
                  {customizations.map((item) => (
                    <tr key={item.id} className="hover:bg-white/60 transition-colors">
                      <td className="py-3 px-3 font-bold text-[#4a3531]">
                        {item.productName}
                      </td>
                      ...
                      <td className="py-3 px-3 font-bold text-[#8c5349] font-[var(--font-cinzel)]">
                        <div className="flex items-center justify-center gap-3">
                          ...
                          <button
                            ...
                            title="Eliminar este producto de tus configuraciones"
                          >
                            Elimina
                          </button>
                        </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 pt-4 border-t border-[#E8DCE2]">
              <p className="text-xs italic text-[#705651] font-medium text-center md:text-left">
                * Tu vela será elaborada a mano cuidando cada detalle de color y aroma elegidos.
              </p>

              <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
                <div className="bg-white/90 border border-[#E8DCE2] px-4 py-2 rounded-xl shadow-2xs text-center md:text-right w-full sm:w-auto">
                  ...
                  <span className="text-lg md:text-xl font-bold text-[#4a3531] tracking-tight font-[var(--font-cinzel)]">
...
export default function CustomizerPage() {
  return (
    <main className="min-h-screen text-[#4a3531] font-[var(--font-montserrat)] p-6 bg-cover bg-center bg-fixed" style={{ backgroundImage: "url('/bg-texture.png')" }}>
