  ...
  const fetchAromasForProduct = async (pId: number) => {
    try {
      ...
      if (!error && relAromas && relAromas.length > 0) {
        ...
        const uniqueAromas = new Map<number, { id: number; name: string }>();

        mapped.forEach((a: { id: number; name: string }) => {
          uniqueAromas.set(a.id, a);
        });

        fetchedAromas = Array.from(uniqueAromas.values());
      }
            ...
            <div className="flex items-center gap-2 overflow-x-auto pt-3 border-t border-[#f0e4e1]/80 px-1">
              ...
              {favoriteProducts.map((fav) => (
                <button
                  ...
                  className={`text-xs px-4 py-1.5 rounded-full font-[var(--font-cinzel)] whitespace-nowrap transition-all ${product.id === fav.id
            ...
            <div className="bg-[#fffcfb] border border-[#f0e4e1] rounded-xl p-4 w-full flex flex-col items-center justify-center relative h-96 shadow-inner overflow-hidden">
              <div className="relative w-48 h-64 flex justify-center items-end my-4">
                ...
                <div className="relative w-36 h-48 rounded-b-[2rem] rounded-t-lg flex flex-col items-center justify-end">
                  ...
                  <div
                    className={`w-full h-40 ${isCustomColor ? '' : `bg-gradient-to-br ${selectedColor.bodyGradient}`
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-2.5">
                ...
                <div className="relative group">
                  ...
                  <div
                    className={`w-8 h-8 rounded-full border-2 border-dashed flex items-center justify-center transition-all ${isCustomColor ? 'border-[#8c5349] bg-white' : 'border-[#a3685e]/50 bg-white/50'
                      }`}
                  >
